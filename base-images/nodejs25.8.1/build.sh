...
NODEJS_FILE="node-v25.8.1-linux-x64.tar.gz"
NODEJS_DIR="node-v25.8.1-linux-x64"
NODEJS_OUTPUT="nodejs.tgz"
IMAGE_NAME="registry.cn-shanghai.aliyuncs.com/odboy/kenaito-cicd:runtime-nodejs25.8.1"

# 判断文件不存在才下载
if [ ! -f "$NODEJS_FILE" ]; then
    echo "NODEJS压缩包不存在，开始下载..."
    curl -O https://nodejs.org/dist/v25.8.1/$NODEJS_FILE
else
