...
NODEJS_FILE="node-v22.22.1-linux-x64.tar.gz"
NODEJS_DIR="node-v22.22.1-linux-x64"
NODEJS_OUTPUT="nodejs.tgz"
IMAGE_NAME="registry.cn-shanghai.aliyuncs.com/odboy/kenaito-cicd:runtime-nodejs22.22.1"

# 判断文件不存在才下载
if [ ! -f "$NODEJS_FILE" ]; then
    echo "NODEJS压缩包不存在，开始下载..."
    curl -O https://nodejs.org/dist/v22.22.1/$NODEJS_FILE
else
